...
export function FileUploadButton({
    ...
    const { registerWorker, workerHandler, workerConstructionError } = useWorker('singleUploadWorker', (e) => {
    ...
    const {
        registerWorker: registerMultipleUploads,
        workerHandler: multipleWorkerHandler,
        workerConstructionError: multipleWorkerConstructionError,
    } = useWorker('multipleUploadsWorker', (e) => {
        setIsCreatingBase64(false);
        globalLoadingStore.getState().removeLoader();
    ...
    const handleAlreadyUploadedImage = useCallback(() => {
        ...
        if (workerHandler && currentValue && !Array.isArray(currentValue)) {
        ...
        if (multipleWorkerHandler && currentValue && Array.isArray(currentValue)) {
            globalLoadingStore.getState().addLoader();
            setIsCreatingBase64(true);
            updateRef.current = true;
    ...
    useEffect(() => {
        if (workerConstructionError || multipleWorkerConstructionError) {
            setRuntimeError(workerConstructionError || multipleWorkerConstructionError);
        }
    }, [workerConstructionError, multipleWorkerConstructionError]);
    ...
    useEffect(() => {
        if (registerWorker && registerMultipleUploads) {
            registerWorker();
            registerMultipleUploads();

            handleAlreadyUploadedImage();
        }
    }, [registerWorker, registerMultipleUploads, workerHandler, multipleWorkerHandler]);
                            ...
                            validateMultiple(tempFiles).then((valid) => {
                                ...
                                if (isSingleFileUpload) {
                                    if (!workerHandler) {
                                        globalLoadingStore.getState().removeLoader();
                                        setRuntimeError(
                                            'Web worker failed to be registered. This is an internal error. Please, try refreshing this page to see if the error will go away',
                                        );
                                        return;
                                    }
