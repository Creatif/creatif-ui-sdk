...
export default function useUpdateList<T>(isUpdateMode: boolean) {
	...
	if (!structureId || !itemId) {
		throw new Error('There are no \'structureId\' or \'itemId\' route parameters in the URL. They must be provided in order for automatic update to work.');
	}
	...
	return async (): Promise<T> => {
		...
		if (error) {
			errorNotification('Query list item error.', 'The system cannot query this list item. Please, try again later.');
