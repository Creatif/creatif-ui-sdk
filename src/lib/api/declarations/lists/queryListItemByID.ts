...
export default function queryListItemByID<Value = unknown, Metadata = unknown>(blueprint: QueryListItemByIDBlueprint) {
	return tryGet<QueriedListItem<Value, Metadata>>(declarations(), `/list/query-id/${Initialize.ProjectID()}/${blueprint.locale ? blueprint.locale : Initialize.Locale()}/${blueprint.structureName}/${blueprint.itemId}`);
}