...
export async function paginateListItems<Value = unknown, Metadata = unknown>(
	blueprint: ListingPagination,
) {
	...
	return await tryGet<PaginationResult<Value, Metadata>>(
		declarations(),
		`/lists/${Initialize.ProjectID()}/${locale}/${
			blueprint.name
		}${queryConstructor(page, limit, groups, orderBy, direction)}`,
	);
}
