...
import type {CreateVariableBlueprint} from '@lib/api/declarations/variableTypes';
export async function createVariable(blueprint: CreateVariableBlueprint) {
	return tryPut(declarations(), `/variable/${Initialize.ProjectID()}/${blueprint.locale ? blueprint.locale : Initialize.Locale()}`, {
		name: blueprint.name,
